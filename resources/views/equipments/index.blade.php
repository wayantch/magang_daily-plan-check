@extends('layouts.app')

@section('content')
    <div class="space-y-6">

        <x-header title="Manage Equipment" />
        <x-alert />

        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-text-primary">Equipment List</h2>
                <p class="text-sm text-text-muted">
                    Manage data center equipments
                </p>
            </div>

            <a href="{{ route('equipments.create') }}"
                class="inline-flex items-center gap-2 px-4 py-2 rounded-lg
                  bg-primary text-accent-foreground font-medium
                  hover:bg-primary-hover transition">
                <i data-lucide="plus"></i>
                Add Equipment
            </a>
        </div>

        <div class="bg-card border border-border rounded-xl overflow-hidden">
            <table class="w-full text-sm">
                <thead class="bg-bg border-b border-border">
                    <tr class="text-text-secondary">
                        <th class="px-4 py-3 text-left">#</th>
                        <th class="px-4 py-3 text-left">Equipment</th>
                        <th class="px-4 py-3 text-left">Category</th>
                        <th class="px-4 py-3 text-left">Room</th>
                        <th class="px-4 py-3 text-left">Status</th>
                        <th class="px-4 py-3 text-right">Action</th>
                    </tr>
                </thead>

                <tbody class="divide-y divide-border">
                    @forelse ($equipments as $equipment)
                        <tr class="hover:bg-bg transition">
                            <td class="px-4 py-3 font-medium uppercase">{{ $loop->iteration }}</td>
                            <td class="px-4 py-3 uppercase">
                                {{ $equipment->name }}
                            </td>

                            <td class="px-4 py-3 uppercase">
                                {{ $equipment->category->name }}
                            </td>
                            {{-- <td class="px-4 py-3 text-text-secondary uppercase">{{ $equipment->room?->name ?? '-' }}</td> --}}
                            <td class="px-4 py-3 text-text-secondary uppercase">
                                {{ $equipment->room?->area?->name ?? '-' }}
                                @if ($equipment->room)
                                    - {{ $equipment->room->name }}
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <span class="text-xs {{ $equipment->is_active ? 'text-success' : 'text-danger' }}">
                                    {{ $equipment->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex gap-2">
                                    <a href="{{ route('equipments.edit', $equipment) }}"
                                        class="p-2 rounded-lg hover:bg-primary-soft">
                                        <i data-lucide="edit"></i>
                                    </a>

                                    <form action="{{ route('equipments.destroy', $equipment) }}" method="POST"
                                        onsubmit="return confirm('Delete this equipment?')">
                                        @csrf
                                        @method('DELETE')
                                        <button class="p-2 rounded-lg hover:bg-danger hover:text-white">
                                            <i data-lucide="trash"></i>
                                        </button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-text-muted">
                                No equipments found
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

            <div class="p-4 border-t border-border bg-bg">
                {{ $equipments->links() }}
            </div>
        </div>

    </div>
@endsection
