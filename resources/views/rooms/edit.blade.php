...
@section('content')
    <div class="space-y-6">
        ...
        <x-header title="Edit Room" />
        <x-alert />

        <form method="POST" action="{{ route('rooms.update', $room->id) }}" class="bg-card border border-border rounded-xl p-6 space-y-4">
            @csrf
            @method('PUT')
            <div>
                ...
                <select name="area_id" class="w-full uppercase mt-1 px-4 py-2 bg-bg border border-border rounded-lg">
                    @foreach ($areas as $area)
                        <option value="{{ $area->id }}" {{ $room->area_id == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                    @endforeach
                </select>
            </div>
